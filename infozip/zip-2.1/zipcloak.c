...
static const char CompiledWith[] = "Compiled with %s%s for %s%s%s.\n\n";
    ...
    char **argv;          /* command line tokens */
{
    ...
    char verify[PWLEN+1]; /* password for encryption or decryption */
    ...
    if (decrypt == 0) {
        if (getp("Verify password: ", verify, PWLEN+1) == NULL)
               ziperr(ZE_PARMS,
                      "stderr is not a tty (you may never see this message!)");
    ...
    if ((start_offset = ftell(outzip)) == -1L) ziperr(ZE_TEMP, tempzip);
    ...
    /* get end of central */
    if ((length = ftell(outzip)) == -1L) ziperr(ZE_TEMP, tempzip);
...
{
    fprintf(stderr, "\
This version of ZipCloak does not support encryption.  Get zcrypt26.zip (or\n\
